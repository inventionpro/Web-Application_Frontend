...
window.openS4DDebugMenu = () => {
    ...
    setTimeout(() => {
      ...
      refreshWorkspaceInformation.onclick = () => {
        ...
        Object.getOwnPropertyNames(workspace).forEach((property) => {
          ...
          keyValue.innerHTML = '<b style="color:#ff8888">' + String(property).replace(/</gim, '&lt;') + '</b>: <b style="color:' + valueColor + '">' + String(appearAs).replace(/</gim, '&lt;') + '</b>';
        ...
        localforage.keys().then((keys) => {
          keys.forEach((key) => {
            localforage.getItem(key).then((value) => {
              ...
              let appearAs = data.value;
              let valueColor = data.color;
              keyValue.innerHTML = '<b style="color:#ff8888">' + String(key).replace(/</gim, '&lt;') + '</b>: <b style="color:' + valueColor + '">' + String(appearAs).replace(/</gim, '&lt;') + '</b>';
  ...
  setTimeout(() => {
    ...
    if (window.location.hostname !== 'localhost' && (typeof process === 'undefined' || !process.env?.VUE_APP_EVALKEY)) {
